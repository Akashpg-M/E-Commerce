...
export const createCheckoutSession = async (req, res) => {
  try{
    ...
    const  lineItems = products.map(product => {
      const amount = Math.round(product.price*100) // stirpe want to send it in the format of cents
    ...
    let coupon = null;
    if(couponCode){
      coupon= await coupon.findOne({code: couponCode, userId:requestAnimationFrame._id, isActive: true});
      if(coupon){
    ...
    //200*100 - coupon if total purcase amount greater than 200 dollar
    if(totalAmount >= 20000) {
      await createNewCoupon(req.user._id)
    }
    res.status(200).json({_id: session.id, totalAmount: totalAmount / 100});
  }catch(error){
...
async function createNewCoupon(userId){
  const newCoupon = new Coupon({
    ...
    expirationDate: new Date(Date.now() + 30*24*60*60*1000), //30 day form now
    userId: userId
  })
